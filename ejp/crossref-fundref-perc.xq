...
let $no-doi := sum(for $x in $list//*:item[@pub-year=$year]/funder[not(@doi)] return if ($x/*:award/*) then count($x/*:award/*) else 1)
let $doi := sum(for $x in $list//*:item[@pub-year=$year]/funder[@doi] return if ($x/*:award/*) then count($x/*:award/*) else 1)
