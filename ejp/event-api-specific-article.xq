...
declare function local:getEventData($cursor as node(),$count as xs:integer) {
   let $response := http:send-request(<http:request method='get' href="{('https://api.eventdata.crossref.org/v1/events?mailto='||$email||'&amp;rows=1000&amp;obj-id=10.7554/eLife.'||$id||'&amp;cursor='||$cursor)}" timeout='30'/>)
