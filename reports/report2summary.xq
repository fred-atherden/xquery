...
let $invalid-total := count($report//*:item[child::*[not(@escape) and @role="error"]])
let $fix-total := count($report//*:item[descendant::*[@action="fix"]])
let $per := $fix-total div $total * 100
let $fixes := count($report//*[@action="fix"])
let $values := for $a in $report//*:item return count($a/*[@action="fix"])
